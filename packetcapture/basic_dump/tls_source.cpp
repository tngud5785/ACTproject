...
#include "certificate_parse.h"

unsigned char* pkcs1_v15_pad(const unsigned char* message, unsigned int message_len, int* padded_len);
unsigned char* der(const unsigned char* m, unsigned int m_len, int* der_len);
		...
		if (th->tls_type == CHANGE_CIPHER_SPEC) {
			...
			if ((ntohs(ap->tls_header.tls_length) == 2 && ap->alert_level == 1) || (ntohs(ap->tls_header.tls_length) == 2 && ap->alert_level == 2)) {
				...
				if (ap->alert_descl == CLOSE_NOTIFY) {
					...
					printf("curve type: %x\n", sd->ske->curve_type);
					printf("\n");
					printf("name curve: %x\n", ntohs(sd->ske->named_curve));
					printf("\n");
					printf("pubkey_length: %x\n", sd->ske->pubkey_length);
					printf("\n");
					printf("pubkey: %s\n", sd->ske_pubkey);
					printf("\n");
					printf("modulus: %s\n", sd->modulus);
					...
					size_t ch_random_len = strlen((const char*)sd->ch_random) / 2;
					size_t sh_random_len = strlen((const char*)sd->sh_random) / 2;
					size_t curve_type_len = sizeof(sd->ske->curve_type);
					size_t named_curve_len = sizeof(sd->ske->named_curve);
					size_t pubkey_length_len = sizeof(sd->ske->pubkey_length);
					size_t pubkey_len = strlen(sd->ske_pubkey) / 2;

					unsigned char* ch_random = (unsigned char*)malloc(ch_random_len);
					unsigned char* sh_random = (unsigned char*)malloc(sh_random_len);
					unsigned char* curve_type = (unsigned char*)sd->ske->curve_type;
					unsigned char* named_curve = (unsigned char*)sd->ske->named_curve;
					unsigned char* pubkey_length = (unsigned char*)sd->ske->pubkey_length;
					unsigned char* pubkey = (unsigned char*)malloc(pubkey_len);

					hex_to_bytes(sd->ch_random, ch_random, ch_random_len);
					hex_to_bytes(sd->sh_random, sh_random, sh_random_len);
					hex_to_bytes(sd->ske_pubkey, pubkey, pubkey_len);


					SHA256_CTX sha256;
					unsigned char hash[SHA256_DIGEST_LENGTH];
					SHA256_Init(&sha256);
					SHA256_Update(&sha256, ch_random, ch_random_len);
					SHA256_Update(&sha256, sh_random, sh_random_len);
					SHA256_Update(&sha256, &curve_type, curve_type_len);
					SHA256_Update(&sha256, &named_curve, named_curve_len);
					SHA256_Update(&sha256, &pubkey_length, pubkey_length_len);
					SHA256_Update(&sha256, pubkey, pubkey_len);
					...
					//err_sign
					BIGNUM* err_sig = NULL;
					BN_hex2bn(&err_sig, sd->signature);
					printf("error sign : ");
					BN_print_fp(stdout, err_sig);
					printf("\n\n");

					int der_len;
					unsigned char* dm = der(hash, SHA256_DIGEST_LENGTH, &der_len);

					//FF padding
					int padded_len;
					unsigned char* pm = pkcs1_v15_pad(dm, der_len, &padded_len);
					BIGNUM* ffpad = BN_bin2bn(pm, padded_len, NULL);
					printf("sign before message(m) : ");
					//BN_print_fp(stdout,ffpad);
					//printf("\n");
					BN_print_fp(stdout, ffpad);
					printf("\n\n");
					...
					if (result == NULL) {
						fprintf(stderr, "Failed to allocate memory for BIGNUM objects or BN_CTX object\n");
						BN_free(err_sig);
						BN_free(e);
						BN_free(exp);
						BN_free(gcd);
					...
					//s^e    
					BN_mod_exp(exp, err_sig, e, n, ctx);
					//s^e-m     
					BN_sub(sub, exp, ffpad);
					//gcd(s^e-m,n)     
					BN_gcd(gcd, sub, n, ctx);

					char* result_hex = BN_bn2hex(gcd);

					printf("Result p: %s\n", result_hex);
					printf("\n\n");
					BN_div(dv, rem, n, gcd, ctx);

					char* result_hexdv = BN_bn2hex(dv);
					printf("Result q: %s\n", result_hexdv);
					printf("\n\n");
			...
			if ((ntohl(hp->handshake_type_leng) >> 24 & 0xFF) == HELLO_REQUEST_RESERVED) {
				...
				sd->ch_random = (const char*)ch_combined_data;
				...
				tls_offset += sizeof(tls_header) + hand_offset;
					...
					sd->sh_random = (const char*)sh_combined_data;
					...
					int total_cert_length = cert->certificates_length[0] << 16 | cert->certificates_length[1] << 8 | cert->certificates_length[2];
					int cert_data_length = cert->certificate_length[0] << 16 | cert->certificate_length[1] << 8 | cert->certificate_length[2];

					char* cert_combined_data = (char*)malloc(cert_data_length * 2 +1);
					memset(cert_combined_data, 0, sizeof(cert_combined_data));
					...
					printf("\n");
					for (int i = 0; i < cert_data_length; i++) {
						sprintf(cert_combined_data + (i * 2), "%02x", cert->certificate_data[i]);
					}

					unsigned char* cert_data = hex_to_bin(cert_combined_data, cert_data_length);
					printf("\n");

					printf("\n");
					parse_certificate(cert_data, cert_data_length);
					...
					sd->ske_pubkey = (const char*)ske_combined_data;
					printf("\n");
					printf("\n");
					...
					char* sial_combined_data = (char*)malloc(ntohs(sial->sign_length) * 2 + 1);
					memset(sial_combined_data, 0, ntohs(sial->sign_length) * 2 + 1);
					...
					for (int i = 0; i < ntohs(sial->sign_length); i++) {
						sprintf(sial_combined_data + (i * 2), "%02x", sial->signature[i]);
					}

					sd->signature = (const char*)sial_combined_data;
					sd->sign_len = (const char*)ntohs(sial->sign_length);
					hand_offset += 4 + (ntohl(hp->handshake_type_leng) & 0xFFFFFF);
					tls_offset += sizeof(tls_header) + hand_offset;
				}
...
unsigned char* pkcs1_v15_pad(const unsigned char* m, unsigned int m_len, int* padded_len) {
	unsigned char* padd = (unsigned char*)malloc(256 * sizeof(unsigned char));
	unsigned char* p = padd;

	*(p++) = 0x00;
	*(p++) = 0x01;
	int fpadd_len = 256 - 3 - m_len;

	memset(p, 0xFF, fpadd_len);
	p += fpadd_len;
	*(p++) = 0x00;
	memcpy(p, m, m_len);
...
//derpadding
unsigned char* der(const unsigned char* m, unsigned int m_len, int* der_len) {
	unsigned char* padd = (unsigned char*)malloc(51 * sizeof(unsigned char));
	unsigned char* p = padd;
