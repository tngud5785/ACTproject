...
void print_tls(const unsigned char* pkt_data) {
	...
	client_hello* ch = (client_hello*)(pkt_data + tls_data);
	...
	if (th->tls_type == CHANGE_CIPHER_SPEC) {
		...
		/*while (ntohl(hp->tls_header.tls_length) != hand_offset) {*/
			hp = (handshake_proto*)(pkt_data + tls_data + hand_offset);
